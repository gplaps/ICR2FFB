...
// -> manual resource management is needed - be careful about possible resource leaks! - that's where C++ matured with patterns like RAII and shared_ptrs, but there are too many differences between C++98 and later to cover everything, which would result in yet another split implementation (e.g. auto_ptr vs shared_ptr)
